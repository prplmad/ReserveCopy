...
{
    public class AbortAndCleanUpBrokenStrategy : IBrokenStrategy
    {
        ...
        public bool CopyFile(string target, string[] source, NLog.Logger logger, string folderFromSource)
        {
            foreach (string filePath in Directory.GetFiles(folderFromSource, "*", SearchOption.AllDirectories))
            {
                ...
                {
                    logger.Error($"\nНе удается скопировать файл {filePath} \nОшибка:" + ex.Message + ". \nЗавершаем работу программы и удаляем то, что успели скопировать!\n");
                    Directory.Delete(target, true);
                    return false;
                }
            }
